...
// Define the Provider class that extends the Sequelize Model class
...
// Initialize the Provider model with its fields and configuration
Provider.init(
  {
  ...
  {
    // Define hooks for the model, specifically a hook to hash the password before creating a new provider
    hooks: {
      async beforeCreate(newProviderData) {
        newProviderData.password = await bcrypt.hash(newProviderData.password, 10); // Hash password with bcrypt
...
// Export the Provider model for use in other parts of the application
